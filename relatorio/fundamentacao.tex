...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners, draw=black!45, fill=black!8,
              align=center, text width=3.1cm, minimum height=9mm, font=\small},
  arr/.style={-{Latex[length=2.2mm]}, thick, black!55},
  darr/.style={-{Latex[length=2.2mm]}, thick, black!55, dashed}
]
\node[box] (p1) {Definição dos parâmetros do filtro};
\node[box, below=of p1] (p2) {Cálculo de polos e zeros analógicos};
\node[box, below=of p2] (p3) {Transformação bilinear};
\node[box, below=of p3] (p4) {Cálculo da função de transferência};
\node[box, right=of p4] (apl) {Aplicação sobre o sinal};
\node[box, above=of apl] (rin) {Recebimento do sinal com ruído};
\node[box, right=of apl] (out) {Sinal filtrado};
\draw[arr] (p1) -- (p2);
\draw[arr] (p2) -- (p3);
\draw[arr] (p3) -- (p4);
\draw[arr] (p4) -- (apl);
...
\caption{Fluxograma do projeto e da aplicação do filtro IIR.}
\label{fig:flowchart}
\end{figure}
